...
install_php7(){
    ...
    if [ `yum list installed | grep php73 | wc -l` -ne 0 ]; then
        echo
    	green "【checked】 PHP7.3安装成功"
	echo
	echo
	sleep 2
	php_status=1
...
install_mysql(){
    ...
    green "==============="
    sleep 2
    echo "nE7jA%5m">/root/password
    mysqlpasswd=$(cat /root/password | head -1 | head -c 9)
...
EOF
    ...
    ~/.acme.sh/acme.sh  --issue  -d $your_domain -d www.$your_domain  --webroot /var/www/html/
...
config_php(){
    ...
    sed -i "s/;listen.owner = nobody/listen.owner = caddy/;" /etc/php-fpm.d/www.conf
    sed -i "s/;listen.group = nobody/listen.group = caddy/;" /etc/php-fpm.d/www.conf
...
install_wp(){
    ...
    echo
    sleep 1
    sed -i "s/database_name_here/wordpress_db/;s/username_here/root/;s/password_here/$mysqlpasswd/;" /var/www/html/wp-config.php
    echo "define('FS_METHOD', "direct");" >> /var/www/html/wp-config.php
    chmod -R 777 /var/www/html/wp-content
    green "==========================================================="
    green " WordPress服务端配置已完成，请打开浏览器访问您的域名进行前台配置"
    green " 数据库密码等信息参考文件：/var/www/html/wp-config.php"
    green "==========================================================="
}
